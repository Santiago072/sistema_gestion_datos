...
-- 1. Crear tabla independiente de proyectos formativos
...
SELECT 
  p.codigo_programa_sofia,
  p.nombre,
  COALESCE(p.nombre_proyecto, p.nombre),
  p.centro_formacion,
  p.regional,
  p.tiempo_estimado_meses,
  p.total_resultados
FROM `programas` p
...
-- 4. Asociar cada programa con su nuevo id_proyecto
UPDATE `programas` p
...
-- 6. Rellenar id_proyecto en fases_proyecto, actividades_fase y fase_competencia_resultado usando el id_ficha previo
...
WHERE fp.id_proyecto IS NOT NULL;
...
WHERE af.id_proyecto IS NOT NULL;
...
ALTER TABLE `programas`
  ADD CONSTRAINT `fk_programas_proyecto` FOREIGN KEY (`id_proyecto`) 
